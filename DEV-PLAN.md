...
> **状态**:Phase 5(V3-a)⏳ Docs create/read/write merged / note design approval required
...
下一步:确认 `/note` 的内部 ID、tag、文件夹 token 与文档域名方案 → 先修 Product-Spec → 建立 Task Contract
```
...
- **工作量**:5–7 天
- **状态**:⏳ Goal active。Issue #29/#33 已完成 FC 可用的文档创建、纯文本读取与正文写入;进入 `/note` 前先确认内部 ID、tag、目录 token 与正确文档域名的最小设计。生产权限仍分开推进。
...
5. **V3-a 正在推进**:Issue #29/#33 与 PR #32/#35 已完成文档创建、读取和根块文本写入;下一步确认 `/note` 设计,再按 `/note` → `/draft`/映射 → `/drafts`/监控拆分;
