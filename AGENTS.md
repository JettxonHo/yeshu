...
**野薯**(Yeshu)是 ENFP 个人管理飞书应用。任务数据在 GitHub Projects V2,内容数据在飞书云文档,通过 GitHub Actions(每日推送)+ 阿里云 FC Worker(实时 /add /today)桥接。
...
# 阿里云 FC 部署凭证不在此处——用 `s config add` 存到 ~/.s/access.yaml
# (上面的 GITHUB_* / LARK_* / AI_* 经 s.yaml environmentVariables 注入 FC 函数)
```
...
A: 改 `worker/s.yaml` 的 `environmentVariables`(`AI_BASE_URL` / `AI_API_KEY` / `AI_MODEL`,本地改 `.env`),代码不动。所有 provider 都支持 OpenAI 兼容 API。
