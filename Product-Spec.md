...
> **迁移说明(2026-07-20 定,2026-07-24 落地)**:Worker 原定 Cloudflare Workers,因 `workers.dev` 国内 DNS 污染(飞书 webhook 入网超时)改阿里云 FC 3.0。实现方式:Hono 应用拆 `createApp`(平台无关)+ `fc.ts` 入口用官方 [`hono-alibaba-cloud-fc3-adapter`](https://github.com/rwv/hono-alibaba-cloud-fc3-adapter)(handler 模型,nodejs20 运行时),esbuild 打成单文件 CJS(`dist/index.js`),Serverless Devs `s deploy` 部署;密钥经 `s.yaml` 的 `environmentVariables`(`${env(KEY)}`)从 `.env` 注入。本地入口 `index.ts` 仍用 `@hono/node-server` 跑常驻服务。GitHub Actions 每日推送(V1-a)不受影响。Cloudflare 代码保留在 git 历史,可回退。详见 DEV-PLAN Phase 2。
